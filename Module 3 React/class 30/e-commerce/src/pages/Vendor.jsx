    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            if(fileUrl){
                const { error } = await supabase
                .from('cars')
                .insert({ name: values.name, model: values.model, price: values.price, description: values.description, image: fileUrl })
                if(error){
                    toast.error(error.message)
                }
            }else{
    ...
    return (
        <div>
            ...
            <form onSubmit={formik.handleSubmit} className='w-[50%] mx-auto mt-8 bg-slate-100 h-[400px] p-4' >
                ...
                <input className='mb-4' accept='/*' onChange={handleFileChange} type="file" />
                {/* {
                    loading ? <Button  type='submit'>Add</Button> :<></>
                } */}
                <Button isDisabled={loading} type='submit'>Add</Button>
            </form>
        </div>
    )
}
